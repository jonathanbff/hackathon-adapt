import { pgTable, uuid, varchar, text, timestamp, integer, real, json, boolean } from "drizzle-orm/pg-core";
...
export const courseGenerationRequests = pgTable("course_generation_requests", {
  ...
  aiPreferences: json("ai_preferences").$type<{
    tone: string;
    interactivity: string;
    examples: string;
    pacing: string;
  }>().notNull(),
  userProfileContext: json("user_profile_context").$type<{
    learningArea: string;
...
export const generationSteps = pgTable("generation_steps", {
  id: uuid("id").primaryKey().defaultRandom(),
  requestId: uuid("request_id").references(() => courseGenerationRequests.id, { onDelete: "cascade" }).notNull(),
  stepNumber: integer("step_number").notNull(),
  stepName: varchar("step_name", { length: 255 }).notNull(),
  status: varchar("status", { length: 50 }).default("pending").notNull(),
  agentId: varchar("agent_id", { length: 100 }).references(() => aiAgents.agentId),
  input: json("input"),
  output: json("output"),
  error: text("error"),
...
export const courseGenerationRequestsRelations = relations(courseGenerationRequests, ({ one, many }) => ({
  user: one(users, {
...
export const insertCourseGenerationRequestSchema = createInsertSchema(courseGenerationRequests);
export const selectCourseGenerationRequestSchema = createSelectSchema(courseGenerationRequests);
...
export const courseGenerationInputSchema = z.object({
  ...
  goals: z.array(z.enum(["career", "skill", "hobby", "certification", "business", "teaching"])),
  duration: z.enum(["1-week", "1-month", "3-months", "6-months"]),
  difficulty: z.enum(["beginner", "intermediate", "advanced"]),
  format: z.array(z.enum(["video", "audio", "text", "interactive", "practical", "visual", "presentation", "quiz"])),
  structure: z.object({
  ...
  userProfileContext: z.object({
    learningArea: z.enum(["technology", "business", "science", "arts", "languages", "health", "education", "others"]),
...
export type CourseGenerationRequest = typeof courseGenerationRequests.$inferSelect;
export type NewCourseGenerationRequest = typeof courseGenerationRequests.$inferInsert;
